...
Do not use the default `python3` for this renderer on this Mac; `pdf2image` is installed in `/Users/vivekgupta/.local/venvs/docx-render`, not in Homebrew's externally managed Python.

Do not replace `/Users/vivekgupta/.local/bin/soffice` with a symlink. It must remain a wrapper that execs the real LibreOffice app-bundle binary, because launching LibreOffice through a symlink caused headless conversion hangs.
